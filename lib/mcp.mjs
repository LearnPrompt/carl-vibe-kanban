...
export const TOOLS = [
  ...
  },
  {
    name: "board_ls",
    description:
      "列出卡片（含 stage、conflicts_with 等派生字段）。Grok 等宿主会截断超过约 19 KB 的返回，多仓时请带 repo 或 limit。",
    inputSchema: {
      type: "object",
      properties: {
        all: { type: "boolean", description: "遍历工作区所有仓库" },
        status: { type: "string", description: "按 status 过滤" },
        repo: { type: "string", description: "按 repo 名过滤，可不配合 all（会自动进入工作区模式）" },
        limit: { type: "number", description: "过滤后只保留前 N 条，避免返回过大被宿主截断" },
      },
      additionalProperties: false,
    },
  },
  ...
  },
  {
    name: "board_judge",
    description:
      "打印对话归档建议表（landed/dup-title/stale-chat 规则常开；ai:true 额外让 claude -p 判 no-clue/dup-title 行）。建议仅供参考，归档仍需用户点头。Grok 等宿主会截断超过约 19 KB 的返回，多仓时请带 repo 或 limit。",
    inputSchema: {
      type: "object",
      properties: {
        all: { type: "boolean", description: "遍历工作区所有仓库" },
        repo: { type: "string", description: "只列有该仓分支线索的行，会自动进入工作区模式" },
        limit: { type: "number", description: "只打印前 N 条建议行，避免返回过大被宿主截断" },
        ai: { type: "boolean", description: "额外调用 claude -p 判定 no-clue/dup-title 行" },
        staleDays: { type: "number", description: "stale-chat 的天数阈值，默认 14" },
      },
      additionalProperties: false,
    },
  },
...
function commandResult(res, { json = false } = {}) {
  ...
  if (json) {
    try {
      ...
      result = textResult(JSON.stringify(parsed));
    } catch {
      result = textResult(stdout);
    }
  } else {
    result = textResult(stdout.trim() || "(no output)");
  }

  // --limit truncation is reported by the CLI as a `LIMIT 显示 N/M` line on
  // stderr (bin/board.mjs). When present, surface it as a SEPARATE trailing
  // content block instead of folding it into the first one — so the data
  // block (JSON array or table text) stays exactly what the CLI printed, and
  // hosts that only look at content[0] never see it mixed in. Every other
  // stderr line (WARN, SKIP, etc.) is ignored here as before.
  const limitLine = stderr.split("\n").find((l) => l.startsWith("LIMIT "));
  if (limitLine) {
    result.content.push({ type: "text", text: limitLine.trim() });
  }
  return result;
}
...
function toolBoardLs(args) {
  ...
  if (args.repo) a.push("--repo", String(args.repo));
...
// board sessions judge --ai shells out to `claude -p` itself and can take a
// while; this tool just mirrors the CLI's table output (plain text, not
// --json — the table shape is the point of this tool, same as sessions ls
// falls back to raw stdout when it isn't asked for JSON).
function toolBoardJudge(args) {
  const a = ["sessions", "judge"];
  if (args.all) a.push("--all");
  if (args.repo) a.push("--repo", String(args.repo));
  if (args.limit != null) a.push("--limit", String(args.limit));
  if (args.ai) a.push("--ai");
  if (args.staleDays != null) a.push("--stale-days", String(args.staleDays));
