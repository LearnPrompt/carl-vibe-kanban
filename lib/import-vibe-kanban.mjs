...
// 0.1.9 之前（本地看板时代）库文件叫 db.sqlite，之后叫 db.v2.sqlite；两个都找，新的优先。
export function defaultDbCandidates() {
  const home = os.homedir();
  let dir;
  if (process.platform === "darwin") {
    dir = path.join(home, "Library", "Application Support", "ai.bloop.vibe-kanban");
  } else if (process.platform === "win32") {
    const appData = process.env.APPDATA || path.join(home, "AppData", "Roaming");
    dir = path.join(appData, "bloop", "vibe-kanban", "data");
  } else {
    const dataHome = process.env.XDG_DATA_HOME || path.join(home, ".local", "share");
    dir = path.join(dataHome, "vibe-kanban");
  }
  return [path.join(dir, "db.v2.sqlite"), path.join(dir, "db.sqlite")];
}
...
export async function readSqliteSource(dbPath, { loadSqliteModule } = {}) {
  ...
  // node 22/23 会打一行 ExperimentalWarning，对用户是噪音；只静音这一类
