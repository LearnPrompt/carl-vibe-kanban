...
// Pure filter used by `board ls` (and reusable anywhere a plain card array
// needs status/flag/repo filtering). `repo` matches the card's `repo` field
// exactly (set by workspace-mode ls to `card.data.repo || probe.repoLabel`) —
// no prefix/substring matching here, unlike sessions.mjs's branch-prefix
// filterRowsByRepo, since a card's repo is a single scalar, not a list.
export function filterCards(cards, { status, flag, repo } = {}) {
  let filtered = cards;
  if (status) filtered = filtered.filter((c) => c.status === status);
  if (flag) filtered = filtered.filter((c) => (c.flags || []).includes(flag));
  if (repo) filtered = filtered.filter((c) => c.repo === repo);
  return filtered;
}

// Pure: truncates `items` to the first `limit` entries. `limit` may be
// undefined/null (no limit — passthrough), a positive integer, or a string
// holding one (CLI flags arrive as strings) — anything else (0, negative,
// non-integer, non-numeric) throws so the caller can report `--limit` misuse
// and exit non-zero rather than silently misbehaving. `total` is the input
// length (i.e. after any upstream filtering, before this truncation), which
// is what callers print in "LIMIT 显示 N/M".
export function applyLimit(items, limit) {
  if (limit === undefined || limit === null) {
    return { items, total: items.length, truncated: false };
  }
  // A bare trailing `--limit` (no value) parses to boolean `true` upstream —
  // treat that as missing-value misuse, not "limit 1".
  const n = typeof limit === "boolean" ? NaN : typeof limit === "number" ? limit : Number(limit);
  if (!Number.isFinite(n) || !Number.isInteger(n) || n <= 0) {
    throw new Error(`--limit 必须是正整数，收到: ${limit}`);
  }
  const total = items.length;
  if (total <= n) return { items, total, truncated: false };
  return { items: items.slice(0, n), total, truncated: true };
}
