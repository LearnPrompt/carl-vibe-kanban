...
import { readHookEventsRaw } from "./hooks.mjs";
...
// --- board-spec-v0.4 §A1: hook-写下的线索（最高优先级） ---------------------
//
// `board hook claude` / `board hook codex` append one JSON line per event to
// `<cacheDir>/hooks/events.jsonl` (see hooks.mjs) the moment a conversation
// touches a repo — no waiting for a transcript scan. Loading/grouping that
// log lives here (not hooks.mjs) because it feeds directly into
// mergeSessionRow below, same as the transcript cache and app-imported
// sessions it now outranks.

// I/O: re-reads hooks.mjs's append-only event log. Delegates the actual
// parsing (skip malformed lines, missing file -> []) to hooks.mjs so there's
// one implementation of "read events.jsonl", not two.
export function loadHookEvents(cacheDir) {
  return readHookEventsRaw(cacheDir);
}

// Pure: groups raw hook events by sessionId. Events with no sessionId (the
// Codex notify payload carried nothing resolvable) are dropped — they can't
// be attributed to any session row.
export function groupHookEventsBySession(events) {
  const map = new Map();
  for (const ev of events || []) {
    if (!ev || !ev.sessionId) continue;
    const arr = map.get(ev.sessionId) || [];
    arr.push(ev);
    map.set(ev.sessionId, arr);
  }
  return map;
}

// Pure: resolves one session's raw hook events (already narrowed to a single
// sessionId) into the branches it touched IN THIS REPO. An event matches this
// repo when either its repoRoot (rev-parse --show-toplevel at hook time) or
// its worktree (--git-common-dir, reversed into a main-worktree root) equals
// gitCtx.repoRoot by realpath — this is what lets a hook fired inside a
// linked worktree (event.repoRoot = the worktree path) still match a repo
// context whose gitCtx.repoRoot is the MAIN worktree (board-spec-v0.4 §A1
// 事件聚合: "按 repoRoot 归到工作区里对应的仓...--git-common-dir 反推主工作树
// 后再和 workspace.repos 对"), while also matching directly when
// workspace.repos itself points AT a worktree override (e.g. goodcaseai's
// feat/board-v0 worktree) whose path equals event.repoRoot exactly.
export function resolveHookBranchesForRepo(hookEventsForSession, gitCtx) {
  if (!hookEventsForSession || hookEventsForSession.length === 0 || !gitCtx.repoRoot) return [];
  const repoRootReal = git.safeRealpath(gitCtx.repoRoot);
  const prHeadRefNames = (gitCtx.prList || []).map((p) => p.headRefName);
  const branches = new Set();
  for (const ev of hookEventsForSession) {
    const candidates = [];
    if (ev.repoRoot) candidates.push(ev.repoRoot);
    if (ev.worktree) candidates.push(git.resolveMainWorktreeRootFromCommonDir(ev.worktree));
    const matchesThisRepo = candidates.some((c) => c && git.safeRealpath(c) === repoRootReal);
    if (!matchesThisRepo) continue;
    if (ev.branch) branches.add(ev.branch);
  }
  return filterBranchesToRepo(Array.from(branches), gitCtx.localBranches, gitCtx.remoteBranches, prHeadRefNames);
}
...
//
// board-spec-v0.4 §A1: `hookEventsForSession` (this session's raw hook.mjs
// events, already narrowed to this uuid — see groupHookEventsBySession) is
// the HIGHEST-priority signal, ahead of transcript clues, cwd, and the
// prNumber fallback below. It doesn't suppress the other signals (their
// branches still get unioned in — more true positives never hurt), but when
// it finds anything, the row is labeled `via: "hook"` and that label wins
// over `prInferred`'s "按 PR 号推断" — a hook event is a fact recorded in the
// moment, not an inference.
export function mergeSessionRow(uuid, scanned, app, gitCtx, hookEventsForSession = []) {
  const prHeadRefNames = (gitCtx.prList || []).map((p) => p.headRefName);
  ...
  const hookBranches = resolveHookBranchesForRepo(hookEventsForSession, gitCtx);

  let branches = [...hookBranches, ...resolveSessionBranches(scanned, gitCtx)];
  ...
  const lastActive =
    (app && app.lastActivityAt) ||
    (scanned && scanned.lastActive) ||
    (hookEventsForSession.length > 0 ? hookEventsForSession[hookEventsForSession.length - 1].ts : null) ||
    null;
  const via = hookBranches.length > 0 ? "hook" : null;
  ...
  return {
    ...
    prInferred,
    via,
    // 供 judge --ai 用：真实首句 + cwd，二者都可能没有（纯 hook-only 行、
    // 或桌面 app 元数据没带 cwd），故不强行给默认值——调用方按存在与否决定
    // 要不要把这个字段塞进发给 AI 的 payload。
    firstPrompt: (scanned && scanned.firstPrompt) || null,
    cwd,
  };
}
...
//
// board-spec-v0.4 §A1: a session can now be known ONLY via hook events — e.g.
// a `SessionStart` hook fires before the transcript file has anything worth
// scanning, or before the desktop app has imported it — so hookGroups is a
// THIRD source of session rows here, not just extra evidence folded into
// rows that already exist from the other two sources.
export function buildSessionRows(cacheDir, gitCtx) {
  const cache = loadSessionsCache(cacheDir);
  const appSessions = loadAppSessions(cacheDir);
  const hookGroups = groupHookEventsBySession(loadHookEvents(cacheDir));
  ...
  for (const [uuid, scanned] of scannedByUuid) {
    rows.push(mergeSessionRow(uuid, scanned, appByUuid.get(uuid) || null, gitCtx, hookGroups.get(uuid) || []));
    seen.add(uuid);
  }
  for (const [uuid, app] of appByUuid) {
    if (seen.has(uuid)) continue;
    rows.push(mergeSessionRow(uuid, null, app, gitCtx, hookGroups.get(uuid) || []));
    seen.add(uuid);
  }
  for (const [uuid, events] of hookGroups) {
    if (seen.has(uuid)) continue;
    rows.push(mergeSessionRow(uuid, null, null, gitCtx, events));
    seen.add(uuid);
  }
  return rows;
}
...
// --- archive suggestions (rule layer, always on) --------------------------------
//
// `suggestArchive` never touches disk or the network — it's a pure function
// over already-judged rows (each needs at minimum { uuid, title, pinned,
// lastActive, judge: { verdict, reason } }), so `board sessions ls`, `board
// render`, and `board sessions judge` can all call it the same way and get
// the same answer. The AI layer (judge --ai, below) only ever ADDS to or
// narrows what this produces — it never runs instead of it.

const NO_TITLE_PLACEHOLDER = "(无标题)";

// trim, collapse whitespace, strip a trailing " (fork)" / "（fork）" (any
// case), case-insensitive compare. Returns "" for anything not worth
// dedup-matching on (empty, or the "(无标题)" placeholder every branchless,
// app-less row shares — treating that as one giant duplicate group would be
// pure noise).
export function normalizeTitleForDup(title) {
  if (typeof title !== "string") return "";
  let t = title.trim().replace(/\s+/g, " ");
  t = t.replace(/\s*\(fork\)\s*$/i, "").replace(/\s*（fork）\s*$/i, "");
  t = t.trim();
  if (!t || t === NO_TITLE_PLACEHOLDER) return "";
  return t.toLowerCase();
}

function timestampOf(dateStr) {
  if (!dateStr) return 0;
  const t = new Date(dateStr).getTime();
  return Number.isNaN(t) ? 0 : t;
}

// Chinese relative-date label for a dup-title reason string. Not meant to be
// exhaustive (no locale plumbing here) — just legible in a CLI table.
export function formatRelativeDate(dateStr, now = Date.now()) {
  if (!dateStr) return "未知时间";
  const ts = timestampOf(dateStr);
  if (!ts) return "未知时间";
  const nowMs = now instanceof Date ? now.getTime() : typeof now === "number" ? now : Date.now();
  const diffDays = Math.floor((nowMs - ts) / 86400000);
...
// Pure: rows -> Map<uuid, {kind, reason, by:"rule"}|null-omitted>. A uuid
// with no suggestion simply isn't a key in the returned Map (callers should
// use `.get(uuid) || null`, never assume every row has an entry).
//
// Precedence, per board-spec judge: landed > dup-title > stale-chat, and a
// `keep` verdict is an absolute veto — it never gets ANY suggestion, dup-title
// included, because a matched-and-still-active branch is real work, not a
// duplicate to throw away just because its title collides with something
// else.
// Only desktop-app sessions (the ones `sessions import` gave an appSessionId)
// are worth flagging as duplicates / stale / AI candidates: transcript-only
// rows are titled by their first prompt, so "继续" repeats a hundred times and
// nothing can archive them from the app anyway. `landed` still applies to all.
export function isAppSession(row) {
  return !!(row && typeof row.appSessionId === "string" && /^local_[A-Za-z0-9-]{1,64}$/.test(row.appSessionId));
}

export function suggestArchive(rows, { now = Date.now(), staleDays = 14 } = {}) {
  const result = new Map();
  const list = Array.isArray(rows) ? rows : [];

  // 1) landed — judge already says this session's branches are all done.
  for (const r of list) {
    if (r.judge && r.judge.verdict === "can-close") {
      result.set(r.uuid, { kind: "landed", reason: "分支已合并，对话可关", by: "rule" });
    }
  }

  // 2) dup-title — group by normalized title, excluding `keep`-verdict rows
  // entirely (the veto above) and rows with no usable title. The most recent
  // row (by lastActive) in each group of 2+ is exempt; everyone else gets
  // dup-title unless they already carry `landed` (which is left standing).
  const groups = new Map();
  for (const r of list) {
    if (!isAppSession(r)) continue;
    if (r.judge && r.judge.verdict === "keep") continue;
    const norm = normalizeTitleForDup(r.title);
    if (!norm) continue;
  ...
  for (const group of groups.values()) {
    ...
    const sorted = [...group].sort((a, b) => timestampOf(b.lastActive) - timestampOf(a.lastActive));
    const newest = sorted[0];
    for (let i = 1; i < sorted.length; i++) {
      const r = sorted[i];
      if (result.has(r.uuid)) continue; // already landed — stays landed
      result.set(r.uuid, {
        kind: "dup-title",
        reason: `与「${newest.title}」重复，较新的一条在 ${formatRelativeDate(newest.lastActive, now)}`,
        by: "rule",
      });
    }
  }

  // 3) stale-chat — no-clue, unpinned, older than staleDays. Never overrides
  // landed/dup-title; `keep` rows are already excluded by construction (they
  // can't be verdict "no-clue" at the same time).
  for (const r of list) {
    if (result.has(r.uuid)) continue;
    if (!isAppSession(r)) continue;
    if (!r.judge || r.judge.verdict !== "no-clue") continue;
    if (r.pinned) continue;
    if (!r.lastActive) continue;
    const days = Math.floor((timestampOf(now) - timestampOf(r.lastActive)) / 86400000);
    if (days >= staleDays) {
      result.set(r.uuid, { kind: "stale-chat", reason: `无分支线索且 ${days} 天没动`, by: "rule" });
    }
  }
...
// --- archive suggestions (AI layer, opt-in) -------------------------------------
//
// `board sessions judge --ai` sends only the rows the rule layer couldn't
// confidently place (no-clue) or already flagged as a probable duplicate
// (dup-title) to `claude -p`, and folds its verdicts back into the same
// suggest Map — never in place of the rule layer, only on top of it.

export function judgeCachePath(cacheDir) {
  return path.join(cacheDir, "judge.json");
}

export function loadJudgeCache(cacheDir) {
  const p = judgeCachePath(cacheDir);
  if (!fs.existsSync(p)) return {};
  try {
    const raw = JSON.parse(fs.readFileSync(p, "utf8"));
...
export function saveJudgeCache(cacheDir, cache) {
  const p = judgeCachePath(cacheDir);
  fs.mkdirSync(path.dirname(p), { recursive: true });
  fs.writeFileSync(p, JSON.stringify(cache, null, 2), "utf8");
}

// Which rows are even eligible for the AI layer: not `keep` (that's a hard
// veto, same as the rule layer), and either `no-clue` or already rule-flagged
// `dup-title`. Capped at `cap` (default 120) so one run can't balloon the
// prompt sent to `claude -p`.
export function selectAiJudgeCandidates(rows, suggestByUuid, { cap = 120 } = {}) {
  const out = [];
  for (const r of rows || []) {
    if (!isAppSession(r)) continue;
    const verdict = r.judge && r.judge.verdict;
    if (verdict === "keep") continue;
    const suggest = suggestByUuid instanceof Map ? suggestByUuid.get(r.uuid) : null;
    const isDup = !!suggest && suggest.kind === "dup-title";
    if (verdict !== "no-clue" && !isDup) continue;
    out.push(r);
...
// Of the eligible candidates, only the ones worth actually re-sending to the
// model: no cache entry yet, or the row's lastActive moved on since it was
// last judged (board-spec: "re-judged only when its lastActive changed").
export function filterRowsNeedingAiJudge(candidateRows, judgeCache) {
  return (candidateRows || []).filter((r) => {
    const cached = judgeCache && judgeCache[r.uuid];
    return !cached || cached.lastActive !== r.lastActive;
  });
}

// One candidate row -> the trimmed shape actually sent to the model.
// firstPrompt/cwdRepo are OMITTED (not sent as null) when unavailable, per
// spec, to keep the prompt small and avoid teaching the model to treat a
// null as meaningful signal.
export function buildAiJudgeRow(row) {
  const out = {
    id: row.uuid,
    title: row.title,
    lastActive: row.lastActive,
    pinned: !!row.pinned,
    verdict: (row.judge && row.judge.verdict) || "no-clue",
  };
  if (row.firstPrompt) out.firstPrompt = String(row.firstPrompt).slice(0, 200);
  if (row.cwd) out.cwdRepo = path.basename(row.cwd);
  return out;
}

// The full prompt string handed to `claude -p --output-format json`. Chinese
// instructions per board-spec judge --ai: one-off Q&A / delivered drafts /
// superseded duplicates -> archive; anything mentioning an unfinished
// plan/next step -> keep; otherwise -> ask. Output contract is a strict JSON
// array so parseAiJudgeResponse below has something predictable to parse.
export function buildAiJudgePrompt(candidateRows) {
  const payload = (candidateRows || []).map(buildAiJudgeRow);
  return [
    "你在帮用户判断哪些 Claude Code / Codex 历史对话可以归档。",
    "下面是候选对话列表（JSON 数组），每条最多包含 id、title、firstPrompt（前 200 字，可能没有）、lastActive、cwdRepo（可能没有）、pinned、verdict：",
    JSON.stringify(payload),
    "",
    "判断标准：",
    "- 一次性问答、已经交付的草稿、被更新对话取代的重复对话，判 archive。",
    "- 提到未完成计划或下一步的，判 keep。",
    "- 拿不准的，判 ask。",
    "",
    "严格只输出一个 JSON 数组，不要任何解释文字、不要代码块标记，形如：",
    '[{"id":"<uuid>","decision":"archive|keep|ask","reason":"<不超过30个汉字的理由>"}]',
  ].join("\n");
}

const VALID_AI_DECISIONS = new Set(["archive", "keep", "ask"]);

function sanitizeAiDecisions(arr) {
  if (!Array.isArray(arr)) return [];
  const out = [];
  for (const item of arr) {
    if (!item || typeof item !== "object") continue;
    if (typeof item.id !== "string" || !item.id) continue;
    if (!VALID_AI_DECISIONS.has(item.decision)) continue;
    out.push({
      id: item.id,
      decision: item.decision,
      reason: typeof item.reason === "string" ? item.reason.slice(0, 60) : "",
    });
  }
  return out;
}

// Parses whatever `claude -p --output-format json` printed. Robust against:
// the `--output-format json` envelope ({"result": "...", ...}, the model's
// own reply as a string inside it), a bare JSON array, code-fenced JSON
// (```json ... ```), stray chatter before/after the array, and outright
// garbage. Never throws — anything that doesn't resolve to a well-formed
// array of {id, decision, reason} decisions comes back as [].
export function parseAiJudgeResponse(raw) {
  if (typeof raw !== "string" || !raw.trim()) return [];
  let text = raw.trim();

  try {
    const envelope = JSON.parse(text);
    if (Array.isArray(envelope)) return sanitizeAiDecisions(envelope);
    if (envelope && typeof envelope === "object" && typeof envelope.result === "string") {
      text = envelope.result;
    }
  } catch {
    // Not itself valid JSON (or not the envelope shape) — fall through and
    // try to find an array embedded in the raw text below.
  }

  // Strip ``` / ```json fences by just dropping every fence marker — leaves
  // the fenced content itself in place for the bracket search that follows.
  ...
  return sanitizeAiDecisions(parsed);
}

// Folds one AI decision into the shared suggest Map, in place, and returns it
// (for chaining). "archive" adds ai-archive UNLESS the row is already
// `landed` (a rule-confirmed merge beats an AI guess); "keep" removes a
// `stale-chat` suggestion specifically (board-spec: AI keep overrides
// stale-chat) but leaves any other existing suggestion (e.g. dup-title)
// alone — the AI here is vouching for "there's still a next step", which
// doesn't un-duplicate a title collision. "ask" (or anything else) changes
// nothing.
export function applyAiDecision(suggestMap, uuid, decision, reason) {
  const current = suggestMap.get(uuid) || null;
  if (decision === "archive") {
    if (!current || current.kind !== "landed") {
      suggestMap.set(uuid, { kind: "ai-archive", reason: reason || "AI 建议归档", by: "ai" });
    }
  } else if (decision === "keep") {
    if (current && current.kind === "stale-chat") {
      suggestMap.delete(uuid);
    }
  }
  return suggestMap;
}

// Applies every still-valid (lastActive-matched) cache entry onto `rows`,
// mutating and returning `suggestMap`. Used by `sessions ls` / `render` (read
// paths that never call the AI themselves) so a previous `judge --ai` run's
// verdicts keep showing up until something about the session actually
// changes.
export function applyJudgeCache(rows, suggestMap, judgeCache) {
  for (const r of rows || []) {
    if (!isAppSession(r)) continue; // transcript-only rows never carry AI verdicts (see isAppSession)
    const cached = judgeCache && judgeCache[r.uuid];
    if (!cached) continue;
    if (cached.lastActive !== r.lastActive) continue; // stale cache entry — needs a re-judge, not blind reuse
    applyAiDecision(suggestMap, r.uuid, cached.decision, cached.reason);
  }
  return suggestMap;
}

// Merges freshly-parsed AI decisions into the on-disk judge cache shape
// ({ [uuid]: { lastActive, decision, reason, judgedAt } }), keyed by each
// judged row's CURRENT lastActive so a later change invalidates it (see
// filterRowsNeedingAiJudge). Pure — does not read/write the cache file
// itself; caller still does loadJudgeCache/saveJudgeCache.
// --- --repo filtering (workspace mode) ---------------------------------------
//
// `board sessions judge --repo <label>` keeps only rows carrying at least one
// branch attributed to that repo. Row `branches` entries are prefixed
// "<repoLabel>:<branch>" by buildWorkspaceSessionRows (bin/board.mjs), so this
// is a prefix match on "<label>:" — deliberately exact on the label segment
// (not a bare substring test) so `goodcaseai` never matches a
// `goodcaseai-lite:` branch entry from a differently-named repo. Rows with no
// branch clue at all (dup-title / stale-chat / ai-archive rows judged purely
// on title/staleness) carry an empty `branches` array and are dropped by
// --repo — that's intentional, not a bug: those rows have no repo to filter by.
export function filterRowsByRepo(rows, label) {
  if (!label) return rows || [];
  const prefix = `${label}:`;
  return (rows || []).filter((r) => (r.branches || []).some((b) => b.startsWith(prefix)));
}

export function mergeJudgeDecisions(judgeCache, rows, decisions) {
  const rowsByUuid = new Map((rows || []).map((r) => [r.uuid, r]));
  const next = { ...(judgeCache || {}) };
  const judgedAt = new Date().toISOString();
  for (const d of decisions || []) {
    const row = rowsByUuid.get(d.id);
    if (!row) continue;
    next[d.id] = { lastActive: row.lastActive, decision: d.decision, reason: d.reason, judgedAt };
  }
  return next;
}
