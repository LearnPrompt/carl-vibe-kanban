...
import {
  ...
  buildSessionRows,
  suggestArchive,
  normalizeTitleForDup,
  formatRelativeDate,
  selectAiJudgeCandidates,
  filterRowsNeedingAiJudge,
  buildAiJudgeRow,
  buildAiJudgePrompt,
  parseAiJudgeResponse,
  applyAiDecision,
  applyJudgeCache,
  mergeJudgeDecisions,
  filterRowsByRepo,
...
// --- suggestArchive: rule layer (board-spec sessions judge) ----------------------
...
function row(overrides) {
  const uuid = (overrides && overrides.uuid) || "u-default";
  return {
    uuid,
    appSessionId: `local_${uuid}`,
    title: "对话",
    pinned: false,
...
test("suggestArchive: transcript-only rows (no appSessionId) only ever get landed", () => {
  const rows = [
    row({ uuid: "t1", appSessionId: null, title: "继续", lastActive: daysAgoIso(30) }),
    row({ uuid: "t2", appSessionId: null, title: "继续", lastActive: daysAgoIso(1) }),
    row({ uuid: "t3", appSessionId: null, judge: { verdict: "can-close", reason: null } }),
  ];
  const suggest = suggestArchive(rows, { now: NOW });
  assert.equal(suggest.get("t1"), undefined);
  assert.equal(suggest.get("t2"), undefined);
  assert.equal(suggest.get("t3").kind, "landed");
  assert.deepEqual(selectAiJudgeCandidates(rows, suggest).map((r) => r.uuid), []);
});

test("suggestArchive: can-close verdict -> landed with fixed reason", () => {
  const rows = [row({ uuid: "a", judge: { verdict: "can-close", reason: null } })];
  const suggest = suggestArchive(rows, { now: NOW });
  assert.deepEqual(suggest.get("a"), { kind: "landed", reason: "分支已合并，对话可关", by: "rule" });
});

test("suggestArchive: dup-title picks the OLDER row(s), leaves the most recent alone", () => {
  const rows = [
    row({ uuid: "old", title: "修复登录 bug", lastActive: daysAgoIso(5) }),
    row({ uuid: "new", title: "修复登录 bug", lastActive: daysAgoIso(1) }),
  ];
  const suggest = suggestArchive(rows, { now: NOW });
  assert.equal(suggest.get("new"), undefined);
  const s = suggest.get("old");
  assert.equal(s.kind, "dup-title");
  assert.equal(s.by, "rule");
  assert.match(s.reason, /^与「修复登录 bug」重复，较新的一条在/);
});

test("suggestArchive: dup-title on 3+ duplicates exempts only the single most-recent one", () => {
  const rows = [
    row({ uuid: "a", title: "写周报", lastActive: daysAgoIso(10) }),
    row({ uuid: "b", title: "写周报", lastActive: daysAgoIso(5) }),
    row({ uuid: "c", title: "写周报", lastActive: daysAgoIso(1) }),
  ];
  const suggest = suggestArchive(rows, { now: NOW });
  assert.equal(suggest.get("c"), undefined);
  assert.equal(suggest.get("a").kind, "dup-title");
  assert.equal(suggest.get("b").kind, "dup-title");
});

test("suggestArchive: dup-title normalizes trailing (fork)/（fork）suffix and case before comparing", () => {
  const rows = [
    row({ uuid: "orig", title: "Refactor Auth Flow", lastActive: daysAgoIso(3) }),
    row({ uuid: "forked", title: "refactor auth flow (fork)", lastActive: daysAgoIso(6) }),
    row({ uuid: "forked-cn", title: "  Refactor Auth Flow （fork）  ", lastActive: daysAgoIso(9) }),
  ];
  const suggest = suggestArchive(rows, { now: NOW });
  assert.equal(suggest.get("orig"), undefined); // most recent of the group
  assert.equal(suggest.get("forked").kind, "dup-title");
  assert.equal(suggest.get("forked-cn").kind, "dup-title");
});

test("suggestArchive: a row already landed keeps landed instead of being overwritten by dup-title", () => {
  const rows = [
    row({ uuid: "merged-old", title: "同一个标题", lastActive: daysAgoIso(5), judge: { verdict: "can-close", reason: null } }),
    row({ uuid: "still-open", title: "同一个标题", lastActive: daysAgoIso(1), judge: { verdict: "no-clue", reason: null } }),
  ];
  const suggest = suggestArchive(rows, { now: NOW });
  assert.equal(suggest.get("merged-old").kind, "landed");
});

test("suggestArchive: keep verdict never gets a suggestion, even with a duplicate title or old age", () => {
  const rows = [
    row({ uuid: "keep-dup-a", title: "重复标题", lastActive: daysAgoIso(30), judge: { verdict: "keep", reason: "有未提交文件" } }),
    row({ uuid: "keep-dup-b", title: "重复标题", lastActive: daysAgoIso(1), judge: { verdict: "keep", reason: "有未提交文件" } }),
  ];
  const suggest = suggestArchive(rows, { now: NOW, staleDays: 1 });
  assert.equal(suggest.size, 0);
});

test("suggestArchive: stale-chat fires only once lastActive is at/past staleDays, for unpinned no-clue rows", () => {
  const freshRow = row({ uuid: "fresh", title: "还没到期的对话", lastActive: daysAgoIso(13) });
  const staleRow = row({ uuid: "stale", title: "过期没人管的对话", lastActive: daysAgoIso(14) });
  const suggest = suggestArchive([freshRow, staleRow], { now: NOW, staleDays: 14 });
  assert.equal(suggest.get("fresh"), undefined);
  assert.deepEqual(suggest.get("stale"), { kind: "stale-chat", reason: "无分支线索且 14 天没动", by: "rule" });
});

test("suggestArchive: stale-chat never fires for a pinned row", () => {
  const rows = [row({ uuid: "pinned-old", pinned: true, lastActive: daysAgoIso(90) })];
  const suggest = suggestArchive(rows, { now: NOW, staleDays: 14 });
  assert.equal(suggest.get("pinned-old"), undefined);
});

test("suggestArchive: stale-chat never fires for a keep or can-close verdict", () => {
  const rows = [
    row({ uuid: "keep-old", lastActive: daysAgoIso(90), judge: { verdict: "keep", reason: "PR #1 还开着" } }),
    row({ uuid: "closed-old", lastActive: daysAgoIso(90), judge: { verdict: "can-close", reason: null } }),
  ];
  const suggest = suggestArchive(rows, { now: NOW, staleDays: 14 });
  assert.equal(suggest.get("keep-old"), undefined);
  assert.equal(suggest.get("closed-old").kind, "landed"); // can-close still gets landed, just not stale-chat
});

// --- normalizeTitleForDup / formatRelativeDate -----------------------------------

test("normalizeTitleForDup: trims, collapses whitespace, strips (fork) suffix, lowercases", () => {
  assert.equal(normalizeTitleForDup("  Fix   Bug (fork)  "), "fix bug");
  assert.equal(normalizeTitleForDup("修一下 Bug （fork）"), "修一下 bug");
  assert.equal(normalizeTitleForDup("(无标题)"), "");
  assert.equal(normalizeTitleForDup(""), "");
  assert.equal(normalizeTitleForDup(null), "");
});

test("formatRelativeDate: today/yesterday/N 天前", () => {
  assert.equal(formatRelativeDate(daysAgoIso(0), NOW), "今天");
  assert.equal(formatRelativeDate(daysAgoIso(1), NOW), "昨天");
  assert.equal(formatRelativeDate(daysAgoIso(5), NOW), "5 天前");
  assert.equal(formatRelativeDate(null, NOW), "未知时间");
});

// --- AI judge layer: candidate selection + response parsing ----------------------

test("selectAiJudgeCandidates: skips keep, includes no-clue and rule dup-title, caps at `cap`", () => {
  const rows = [
    row({ uuid: "keep-1", judge: { verdict: "keep", reason: "x" } }),
    row({ uuid: "no-clue-1", judge: { verdict: "no-clue", reason: null } }),
    row({ uuid: "can-close-1", judge: { verdict: "can-close", reason: null } }),
  ];
  const suggestByUuid = new Map([["can-close-1", { kind: "landed", reason: "x", by: "rule" }]]);
  const candidates = selectAiJudgeCandidates(rows, suggestByUuid, { cap: 120 });
  assert.deepEqual(candidates.map((r) => r.uuid), ["no-clue-1"]);
});

test("selectAiJudgeCandidates: respects cap", () => {
  const rows = Array.from({ length: 5 }, (_, i) => row({ uuid: `u${i}`, judge: { verdict: "no-clue", reason: null } }));
  const candidates = selectAiJudgeCandidates(rows, new Map(), { cap: 2 });
  assert.equal(candidates.length, 2);
});

test("filterRowsNeedingAiJudge: skips rows whose cache entry's lastActive still matches", () => {
  const rows = [row({ uuid: "fresh-cache", lastActive: "2026-09-01T00:00:00Z" }), row({ uuid: "no-cache", lastActive: "2026-09-01T00:00:00Z" })];
  const cache = { "fresh-cache": { lastActive: "2026-09-01T00:00:00Z", decision: "archive", reason: "x", judgedAt: "x" } };
  const needing = filterRowsNeedingAiJudge(rows, cache);
  assert.deepEqual(needing.map((r) => r.uuid), ["no-cache"]);
});

test("filterRowsNeedingAiJudge: re-includes a row whose lastActive changed since it was cached", () => {
  const rows = [row({ uuid: "moved", lastActive: "2026-09-05T00:00:00Z" })];
  const cache = { moved: { lastActive: "2026-09-01T00:00:00Z", decision: "keep", reason: "x", judgedAt: "x" } };
  const needing = filterRowsNeedingAiJudge(rows, cache);
  assert.deepEqual(needing.map((r) => r.uuid), ["moved"]);
});

test("buildAiJudgeRow: omits firstPrompt/cwdRepo entirely when unavailable, includes when present", () => {
  const bare = buildAiJudgeRow(row({ uuid: "bare" }));
  assert.ok(!("firstPrompt" in bare));
  assert.ok(!("cwdRepo" in bare));

  const rich = buildAiJudgeRow(row({ uuid: "rich", firstPrompt: "帮我写个脚本", cwd: "/Users/carl/projects/goodcaseai" }));
  assert.equal(rich.firstPrompt, "帮我写个脚本");
  assert.equal(rich.cwdRepo, "goodcaseai");
});

test("buildAiJudgePrompt: is Chinese, embeds the candidate JSON, and demands a strict JSON array back", () => {
  const prompt = buildAiJudgePrompt([row({ uuid: "p1" })]);
  assert.match(prompt, /archive/);
  assert.match(prompt, /"id":"p1"/);
  assert.match(prompt, /JSON 数组/);
});

test("parseAiJudgeResponse: bare JSON array", () => {
  const decisions = parseAiJudgeResponse('[{"id":"a","decision":"archive","reason":"一次性问答"}]');
  assert.deepEqual(decisions, [{ id: "a", decision: "archive", reason: "一次性问答" }]);
});

test("parseAiJudgeResponse: `claude -p --output-format json` envelope with the array embedded in .result", () => {
  const envelope = JSON.stringify({
    type: "result",
    subtype: "success",
    result: '[{"id":"b","decision":"keep","reason":"提到下一步"}]',
  });
  const decisions = parseAiJudgeResponse(envelope);
  assert.deepEqual(decisions, [{ id: "b", decision: "keep", reason: "提到下一步" }]);
});

test("parseAiJudgeResponse: fenced ```json ... ``` block, with chatter around it", () => {
  const raw = [
    "好的，这是我的判断：",
    "```json",
    '[{"id":"c","decision":"ask","reason":"看不出来"}]',
    "```",
    "希望有帮助！",
  ].join("\n");
  const decisions = parseAiJudgeResponse(raw);
  assert.deepEqual(decisions, [{ id: "c", decision: "ask", reason: "看不出来" }]);
});

test("parseAiJudgeResponse: plain ``` fence (no json tag) around the array still parses", () => {
  const raw = "```\n[{\"id\":\"d\",\"decision\":\"archive\",\"reason\":\"重复\"}]\n```";
  const decisions = parseAiJudgeResponse(raw);
  assert.deepEqual(decisions, [{ id: "d", decision: "archive", reason: "重复" }]);
});

test("parseAiJudgeResponse: garbage input -> empty array, never throws", () => {
  assert.deepEqual(parseAiJudgeResponse("not json at all"), []);
  assert.deepEqual(parseAiJudgeResponse(""), []);
  assert.deepEqual(parseAiJudgeResponse(null), []);
  assert.deepEqual(parseAiJudgeResponse("{}"), []);
  assert.deepEqual(parseAiJudgeResponse("[not, valid, json]"), []);
});

test("parseAiJudgeResponse: drops entries with an invalid decision or missing id, keeps the valid ones", () => {
  const raw = JSON.stringify([
    { id: "ok", decision: "archive", reason: "x" },
    { id: "bad-decision", decision: "delete-forever" },
    { decision: "archive", reason: "no id" },
    "not even an object",
  ]);
  const decisions = parseAiJudgeResponse(raw);
  assert.deepEqual(decisions, [{ id: "ok", decision: "archive", reason: "x" }]);
});

// --- applyAiDecision / applyJudgeCache / mergeJudgeDecisions ---------------------

test("applyAiDecision: archive adds ai-archive when nothing stronger exists", () => {
  const suggest = new Map();
  applyAiDecision(suggest, "u1", "archive", "一次性问答已交付");
  assert.deepEqual(suggest.get("u1"), { kind: "ai-archive", reason: "一次性问答已交付", by: "ai" });
});

test("applyAiDecision: archive never overrides an existing landed suggestion", () => {
  const suggest = new Map([["u1", { kind: "landed", reason: "分支已合并，对话可关", by: "rule" }]]);
  applyAiDecision(suggest, "u1", "archive", "AI 觉得能关");
  assert.equal(suggest.get("u1").kind, "landed");
});

test("applyAiDecision: keep removes a stale-chat suggestion", () => {
  const suggest = new Map([["u1", { kind: "stale-chat", reason: "无分支线索且 20 天没动", by: "rule" }]]);
  applyAiDecision(suggest, "u1", "keep", "提到了下一步");
  assert.equal(suggest.has("u1"), false);
});

test("applyAiDecision: keep leaves a dup-title suggestion alone (only stale-chat is overridden)", () => {
  const suggest = new Map([["u1", { kind: "dup-title", reason: "重复", by: "rule" }]]);
  applyAiDecision(suggest, "u1", "keep", "其实不是重复");
  assert.equal(suggest.get("u1").kind, "dup-title");
});

test("applyAiDecision: 'ask' changes nothing", () => {
  const suggest = new Map();
  applyAiDecision(suggest, "u1", "ask", "看不出来");
  assert.equal(suggest.has("u1"), false);
});

test("applyJudgeCache: applies a cache entry only when lastActive still matches the row", () => {
  const rows = [row({ uuid: "u1", lastActive: "2026-09-01T00:00:00Z" })];
  const cache = { u1: { lastActive: "2026-09-01T00:00:00Z", decision: "archive", reason: "旧对话", judgedAt: "x" } };
  const suggest = new Map();
  applyJudgeCache(rows, suggest, cache);
  assert.equal(suggest.get("u1").kind, "ai-archive");
});

test("applyJudgeCache: ignores a stale cache entry whose lastActive no longer matches", () => {
  const rows = [row({ uuid: "u1", lastActive: "2026-09-05T00:00:00Z" })];
  const cache = { u1: { lastActive: "2026-09-01T00:00:00Z", decision: "archive", reason: "旧对话", judgedAt: "x" } };
  const suggest = new Map();
  applyJudgeCache(rows, suggest, cache);
  assert.equal(suggest.has("u1"), false);
...
test("mergeJudgeDecisions: keys the new cache entry by the row's CURRENT lastActive", () => {
  const rows = [row({ uuid: "u1", lastActive: "2026-09-09T00:00:00Z" })];
  const next = mergeJudgeDecisions({}, rows, [{ id: "u1", decision: "archive", reason: "一次性问答" }]);
  assert.equal(next.u1.lastActive, "2026-09-09T00:00:00Z");
  assert.equal(next.u1.decision, "archive");
  assert.ok(next.u1.judgedAt);
});

test("mergeJudgeDecisions: preserves untouched existing cache entries", () => {
  const existing = { other: { lastActive: "x", decision: "keep", reason: "y", judgedAt: "z" } };
  const rows = [row({ uuid: "u1", lastActive: "2026-09-09T00:00:00Z" })];
  const next = mergeJudgeDecisions(existing, rows, [{ id: "u1", decision: "archive", reason: "x" }]);
  assert.deepEqual(next.other, existing.other);
});

// --- board-repo-limit: filterRowsByRepo -------------------------------------

test("filterRowsByRepo: keeps only rows with a branch prefixed '<label>:'", () => {
  const rows = [
    row({ uuid: "u1", branches: ["goodcaseai:feat/a"] }),
    row({ uuid: "u2", branches: ["radar:feat/b"] }),
    row({ uuid: "u3", branches: ["goodcaseai:feat/c", "radar:feat/d"] }),
  ];
  const result = filterRowsByRepo(rows, "goodcaseai");
  assert.deepEqual(result.map((r) => r.uuid), ["u1", "u3"]);
});

test("filterRowsByRepo: exact label-prefix match — 'goodcaseai' never matches 'goodcaseai-lite:' branches", () => {
  const rows = [
    row({ uuid: "u1", branches: ["goodcaseai-lite:feat/x"] }),
    row({ uuid: "u2", branches: ["goodcaseai:feat/y"] }),
  ];
  const result = filterRowsByRepo(rows, "goodcaseai");
  assert.deepEqual(result.map((r) => r.uuid), ["u2"]);
});

test("filterRowsByRepo: drops rows with no branch clue at all (dup-title/stale-chat/ai-archive rows)", () => {
  const rows = [row({ uuid: "u1", branches: [] }), row({ uuid: "u2" })];
  const result = filterRowsByRepo(rows, "goodcaseai");
  assert.deepEqual(result, []);
});

test("filterRowsByRepo: a falsy label is a no-op (returns the input as-is)", () => {
  const rows = [row({ uuid: "u1", branches: ["radar:feat/a"] })];
  assert.deepEqual(filterRowsByRepo(rows, undefined), rows);
  assert.deepEqual(filterRowsByRepo(rows, null), rows);
  assert.deepEqual(filterRowsByRepo(rows, ""), rows);
});

test("filterRowsByRepo: null/undefined rows never throws", () => {
  assert.deepEqual(filterRowsByRepo(null, "goodcaseai"), []);
  assert.deepEqual(filterRowsByRepo(undefined, "goodcaseai"), []);
});
