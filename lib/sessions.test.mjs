...
test("mergeSessionRow: prNumber guard (b) — no transcript found, app title matches a board.config.json alias, flagged prInferred", () => {
  const app = { sessionId: "local_feac66d1-8917-4f14-aaee-a9aa68c2136e", title: "图生视频提示语案例 goodcase 复盘", prNumber: 150, pinned: true };
  const row = mergeSessionRow(stripLocalPrefix(app.sessionId), null, app, baseGitCtx);
  assert.deepEqual(row.branches, ["feat/i2v-input-assets"]);
  assert.equal(row.matchedViaTranscript, false);
  assert.equal(row.prInferred, true);
});

test("mergeSessionRow: cross-repo session is rejected — no transcript, title names an unrelated app (aimap), prNumber collides with this repo's PR #2", () => {
  const crossRepoGitCtx = {
    ...baseGitCtx,
    prList: [...prList, { number: 2, headRefName: "feat/should-not-match", state: "OPEN" }],
  };
  const app = { sessionId: "local_x3", title: "aimap", prNumber: 2 };
  const row = mergeSessionRow("x3", null, app, crossRepoGitCtx);
