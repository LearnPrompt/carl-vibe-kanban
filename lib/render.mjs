...
function renderConvChip(session, generatedAt) {
  const pinnedMark = session.pinned ? `<span class="pinned-mark mono">置顶</span>` : "";
  const hookMark = session.via === "hook" ? `<span class="hook-mark mono">hook</span>` : "";
  ...
  return `<details class="conv-chip">
  <summary>
  ...
  <div class="conv-reason mono">${reason}</div>
</details>`;
}

function renderConvListFlat(sessions, generatedAt) {
  return sessions.map((s) => renderConvChip(s, generatedAt)).join("\n");
}
...
function renderProjectSection(project, index, generatedAt) {
  ...
  sortedCards.forEach((card) => {
    ...
    const convsInnerHtml = hasConv
      ? renderConvListFlat(rowsForBranch, generatedAt)
  ...
  if (landedSessions.length) {
    ...
    const visible = landedSessions.length > 8 ? landedSessions.slice(0, 8) : landedSessions;
    const rest = landedSessions.length > 8 ? landedSessions.slice(8) : [];
    const restHtml = rest.length
      ? `<details class="group-details nested">
  <summary>其余未展开的（${rest.length}）</summary>
  <div class="conv-list-flat">${renderConvListFlat(rest, generatedAt)}</div>
</details>`
      : "";
    rowsHtml.push(renderBranchRow(landedNodeHtml, `${renderConvListFlat(visible, generatedAt)}${restHtml}`, true));
  }
...
function renderTreeBody({ projects, noClueSessions, generatedAt }) {
  ...
  <div class="conv-list-flat">${renderConvListFlat(noClue, generatedAt)}</div>
</details>`
    : "";
  return `${sections}\n${noClueHtml}`;
}
