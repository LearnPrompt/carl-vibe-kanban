...
test("default db candidates: db.v2.sqlite first, then 0.1.8-era db.sqlite; defaultDbPath picks one of them", () => {
  if (process.platform !== "darwin") return;
  const cands = defaultDbCandidates();
  assert.match(cands[0], /Library\/Application Support\/ai\.bloop\.vibe-kanban\/db\.v2\.sqlite$/);
  assert.match(cands[1], /Library\/Application Support\/ai\.bloop\.vibe-kanban\/db\.sqlite$/);
  assert.ok(cands.includes(defaultDbPath()));
});
